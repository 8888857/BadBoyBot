...
class FeedBack(commands.Cog):
    ...
    async def _idea(self, ctx, * , idea):
        ...
        if ctx.author.id in self.client.black_list:
            ...
            await ctx.reply(embed = discord.Embed(description=f"**{ctx.message.author.mention}**,\n спасибо за ваш вклад в развитие бота", colour=self.client.COLORS['BASE']))
            await ctx.message.add_reaction(self.client.EMOJIS['SUCCESS'])
            await msg.add_reaction(self.client.EMOJIS['SUCCESS'])
            await msg.add_reaction(self.client.EMOJIS['offline'])
            await msg.add_reaction(self.client.EMOJIS['ERROR'])
