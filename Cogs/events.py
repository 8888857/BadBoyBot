...
class events(commands.Cog):
    ...
    async def on_command_error(self, ctx, error):
        if type(error) == discord.ext.commands.errors.MissingRequiredArgument:
            await ctx.message.add_reaction("❌")
            return await ctx.reply(embed = discord.Embed(title = "Неверно указаны аргументы команды", description = f"Использование команды: {ctx.prefix}{ctx.command.usage}", colour = config.COLORS['ERROR']))
            ...
            await ctx.message.add_reaction("❌")
            return await ctx.reply(embed = discord.Embed(title = "Произошла ошибка", description = "У вас недостаточно прав для выполнения данной команды", colour = config.COLORS['ERROR']))
        elif type(error) == discord.ext.commands.errors.CommandOnCooldown:
            await ctx.message.add_reaction("⏳")
            return await ctx.reply(embed = discord.Embed(title = "У вас кулдаун на эту команду", description = f"Подождите {round(error.retry_after)} секунд, до тех пор, когда вы сможете использовать команду", colour = config.COLORS['ERROR']))
        elif type(error) == discord.ext.commands.errors.NoPrivateMessage:
            await ctx.message.add_reaction("❌")
            return await ctx.reply(embed = discord.Embed(title = "Произошла ошибка", description = "Эту команду можно использовать только на сервере", colour = config.COLORS['ERROR']))
        elif type(error) == discord.ext.commands.errors.MemberNotFound:
            await ctx.message.add_reaction("❌")
            return await ctx.reply(embed = discord.Embed(title = "Указанный пользователь не найден", colour = config.COLORS['ERROR']))
        elif type(error) == discord.ext.commands.errors.ChannelNotFound:
            await ctx.message.add_reaction("❌")
            return await ctx.reply(embed = discord.Embed(title = "Указанный канал не найден", colour = config.COLORS['ERROR']))
        elif type(error) == discord.ext.commands.errors.RoleNotFound:
            await ctx.message.add_reaction("❌")
            return await ctx.reply(embed = discord.Embed(title = "Указанная роль не найдена", colour = config.COLORS['ERROR']))
        elif type(error) == discord.ext.commands.errors.EmojiNotFound:
            await ctx.message.add_reaction("❌")
            return await ctx.reply(embed = discord.Embed(title = "Указанный эмоджи не найден", colour = config.COLORS['ERROR']))
            ...
            await ctx.message.add_reaction("❌")
            return await ctx.reply(embed = discord.Embed(title = "У меня недостаточно прав для выполнения данной команды", description = "Рекомендуется выдать мне права администратора для использования всего функционала бота", colour = config.COLORS['ERROR']))
        elif type(error) == discord.ext.commands.errors.MissingPermissions:
            await ctx.message.add_reaction("❌")
            return await ctx.reply(embed = discord.Embed(title = "У вас недостаточно прав для выполнения этой команды", colour = config.COLORS['ERROR']))
        else:
            if ctx.author.id not in [owner.id for owner in self.client.owners]:
                ...
                await ctx.reply(embed=discord.Embed(description="Произошла непредвиденная ошибка. Отчёт об ошибке уже отправлен разработчикам. Приносим свои извинения за доставленные неудобства.",colour=config.COLORS['ERROR']))
            else:
                await ctx.message.add_reaction("❌")
